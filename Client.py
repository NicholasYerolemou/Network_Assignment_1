...
lblName = tk.Label(serverWindow, text="Please enter your name\nin order to connect to the server.", font=('Helvatical bold',10), bg = 'WHITE', fg = '#a83f2c').pack(side=tk.TOP)
lblName = tk.Label(serverWindow, text="YOUR NAME:", bg = 'WHITE').pack(side=tk.LEFT)
entName = tk.Entry(serverWindow, bg = 'WHITE')
entName.pack(side=tk.LEFT)


btnConnect = tk.Button(serverWindow, text="CONNECT",bg = 'WHITE', width = 20, command=lambda: connectServer(entName.get()))
...
def menu():
    ...
    btnCreateChat = tk.Button(window, text="START NEW CHAT", width=200, height = 5, bg = '#a83f2c', fg = 'WHITE', font=('Helvatical bold',15), command=lambda: newChat())
    btnCreateChat.pack(side=tk.TOP)
    
    btnCreateChat = tk.Button(window, text="OPEN EXISTING CHAT", width=200, height = 5, bg = '#a83f2c', fg = 'WHITE', font=('Helvatical bold',15), command=lambda: openChat())
    btnCreateChat.pack(side=tk.TOP)
    
    btnCreateChat = tk.Button(window, text="EXIT", width=200, height = 5, bg = '#a83f2c', fg = 'WHITE', font=('Helvatical bold',15), command=lambda:  exitApp())
    btnCreateChat.pack(side=tk.TOP)
...
def newChat():
    ...
    btnConnect = tk.Button(topFrame, text="CONNECT",command=lambda: connect(entIP.get(), entIP, btnConnect, tkMessage))
    btnConnect.pack(side=tk.LEFT)
